/*
 * CANONICAL VERSION — @edumind/footer v1.0.0
...
export interface EDUmindFooterProps {
  ...
  /** URL del repositorio público con el código fuente. Default: organización GitHub edumind-es */
  repoUrl?: string;
...
export default function EDUmindFooter({
  ...
  showSocial = true,
  showLegal = true,
  repoUrl = 'https://github.com/edumind-es',
}: EDUmindFooterProps) {
  ...
  const technicalVersion = versionStage
    ? `v${version} (${versionStage})`
    : `v${version}`;
  const versionBadge = versionStage
    ? `v${visibleVersion} · ${versionStage}`
    : `v${visibleVersion}`;
  ...
  return (
    <footer
      className={[
        'edumind-footer',
        compact ? 'edumind-footer--compact' : '',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
    ...
    >
      ...
      <div className="edumind-footer__main">
        <div className="edumind-footer__info">
          ...
          <p className="edumind-footer__license">
            {t.openSource}{' '}
      ...
      {showLegal && !compact && (
        <div className="edumind-footer__trademark">
          <p>
            {t.trademark}{' '}
            <a
              href="mailto:contacto@edumind.es"
      ...
      <div className="edumind-footer__meta">
        {showVersion && (
          <span
            className="edumind-footer__version-badge"
            title={`${t.technicalVersion}: ${technicalVersion}`}
          >
            {versionBadge}
          </span>
